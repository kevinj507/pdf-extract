...
var ocrToSearchablePdf = require('./ocr_to_searchable_pdf');
          ...
          function (pdf_file, cb) {
            ...
            var runOcr = function(tif_path) {
              ...
              if (options.outputSearchablePdf) {
                ...
                var fn = function(error, single_search_pdf_path) {
                  if (error) {
                    err += ', error removing temporary tif file: "'+ error +'"';
                  }
                ...
                ocrToSearchablePdf(tif_path, options, function(err, single_search_pdf_path) {
                  var tif_path_delete = tif_path;
                  if (tif_path_delete) {
                    fs.unlink(tif_path, function (tif_cleanup_err, reply) {
                      fn(tif_cleanup_err, single_search_pdf_path);
                    });
                  } else {
                    fn(err, single_search_pdf_path);
                  }
                });
              } else {
                var ocr_flags = [
            ...
            convert(pdf_file.file_path, quality, function (err, dirty_tif_path) {
              if (err) { return cb(err); }

              if (options.doTextCleaning) { // require textcleaner script to be configure
                clean(dirty_tif_path, options.textCleanerFlags, function(err, tif_path) {
                  if (err) { return cb(err); }
                  runOcr(tif_path);
                });
              } else {
                runOcr(dirty_tif_path);
              }
            });
          }, function (err) {
            if (err) {
