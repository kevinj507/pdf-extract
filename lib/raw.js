          ...
          function (pdf_file, cb) {
            ...
            convert(pdf_file.file_path, quality, function (err, tif_path) {
              ...
              if (options.outputSearchablePdf) {
                ocrToSearchPdf(tif_path, function(err, searchablePdfPath) {
                  fs.unlink(tif_path, function (tif_cleanup_err, reply) {
                    ...
                    single_page_pdf_file_paths.push(searchablePdfPath);
                    self.emit('page', { hash: hash, text: null, index: index, num_pages: num_pages, pdf_path: pdf_path, single_page_pdf_path: searchablePdfPath});
                    text_pages.push('');
                    index++;
                    return cb();
                  });
